...
実 Google Classroom データを読む GUI / CLI を動かす場合は、先に Google API クライアント一式を含む `google` extra を同期してください。
これにより Classroom API を呼び出す `google-api-python-client`、OAuth 用の `google-auth` / `google-auth-oauthlib` などがインストールされます。
...
Classroom 実データの取得や投稿には、Google API クライアントライブラリが必要です。
未インストールの場合は、OAuth や Classroom API の実行前に次を実行してください。
