...
モック実装(`Mock*`)を実Google APIへ切り替える手順。ローカル CLI のみなら Desktop app、別端末ブラウザから GUI を使うなら Web application を使う。
...
5. Web application を使う場合は Authorized redirect URI に `https://<host>/oauth/google/callback` または `http://<host>:<port>/oauth/google/callback` を追加する。
...
- GUI の `/api/live/oauth/*` フローは server-side callback を使うため、別端末ブラウザから使う場合は Web application クライアントが必要。
